...
class Object {
    ...
    snap() {
        this.x = Math.min(Math.max(Math.round(this.x), 1), renderer.width - 2);
        this.y = Math.min(Math.max(Math.round(this.y), 0), renderer.height - 1);
    }
...
class Box extends Object {
    ...
    snap() {
        super.snap();
        this.y = Math.min(Math.max(Math.round(this.y), 0), renderer.height - 2);
    }
...
class Render extends Renderer {
    constructor() {
        ...
        this.templateObjects = [
            ...
            new Output("LED", 0, 0.25, -11.5, this)
            ];
    ...
    load(rrs) {
        ...
        widthInput.addEventListener("change", () => {
            if (!widthInput.value) widthInput.value = this.width;
            this.width = Math.max(parseFloat(widthInput.value), 3);
        ...
        heightInput.addEventListener("change", () => {
            if (!heightInput.value) heightInput.value = this.height;
            this.height = Math.max(parseFloat(heightInput.value), 1);
            heightInput.value = this.height;
            for (let i = 0; i < this.objects.length; i++) {
                let object = this.objects[i];
                if (object instanceof Box) {
                    if (object.y >= this.height - 1) {
                        this.objects.splice(i, 1);
                        i--;
                    }
                }
        ...
        exportFileButton.addEventListener("click", () => {
            let data = this.objectsToString();
            let link = document.createElement("a");
            link.style.display = "none";
            link.setAttribute("href", "data:text/plain;charset=utf-8," + encodeURIComponent(data));
            link.setAttribute("download", "nandu.txt");
    ...
    draw(graph, deltaTime) {
        ...
        let maxX = (-graph.canvas.width / 2) / graph.zoom + graph.convBackFromSW(300);
        let minX = (graph.canvas.width / 2) / graph.zoom - graph.convBackFromSW(425) + this.width;
        let maxY = (-graph.canvas.height / 2) / graph.zoom + graph.convBackFromSH(300);
        let minY = (graph.canvas.height / 2) / graph.zoom - graph.convBackFromSH(300) + this.height;
        if (this.draggingCamera) {
            this.camera.x = Math.min(Math.max(this.camera.x + dx, maxX), minX);
            this.camera.y = Math.min(Math.max(this.camera.y + dy, maxY), minY);
        } else {
            this.camera.x = Math.min(Math.max(this.camera.x, maxX), minX);
            this.camera.y = Math.min(Math.max(this.camera.y, maxY), minY);
        }
        ...
        graph.drawRect(0, 0, this.width, this.height, "black");
        
        let color = "#f7defc";
        if (this.draggingObject && this.draggingObject instanceof Box) {
            color = "white";
        }
        graph.fillRect(0, 0, this.width, this.height, color);
        
        color = "#fcfce3";
        if (this.draggingObject && this.draggingObject instanceof Input) {
            color = "white";
        }
        graph.fillRect(0, 0, 1, this.height, color);

        color = "#fcfce3";
        if (this.draggingObject && this.draggingObject instanceof Output) {
            color = "white";
        }
        graph.fillRect(this.width - 1, 0, this.width, this.height, color);
    ...
    readIn(data) {
        ...
        this.width = parseFloat(lines[0].split(" ")[1]);
        this.height = parseFloat(lines[0].split(" ")[0]);
        widthInput.value = this.width;
        heightInput.value = this.height;
        ...
        for (let x = 1; x < lines.length; x++) {
            ...
            for (let y = 0; y < values.length; y++) {
                ...
                if (value.startsWith("Q")) {
                    ...
                    this.objects.push(new Input("Quelle", id, x - 1, y));
                    continue;
                }
                if (value == "W") {
                    this.addObject(x - 1, y, 0);
                    y += 1;
                    continue;
                }
                if (value == "r") {
                    this.addObject(x - 1, y, 1);
                    y += 1;
                    continue;
                }
                if (value == "R") {
                    this.addObject(x - 1, y, 2);
                    y += 1;
                    continue;
                }
                if (value == "B") {
                    this.addObject(x - 1, y, 3);
                    y += 1;
                    continue;
                }
                if (value.startsWith("L")) {
                    let id = value.substring(1);
                    this.objects.push(new Output("L", id, x - 1, y));
                    continue;
                }
    ...
    getObjectAt(x, y) {
        for (let i = 0; i < this.objects.length; i++) {
            ...
            if (object instanceof Box && object.x == x && object.y == y - 1) {
                return object;
            }
        }
        return null;
    ...
    objectsToString() {
        let string = "" + this.height + " " + this.width + "\n";
        for (let x = 0; x < this.width; x++) {
            for (let y = 0; y < this.height; y++) {
                ...
                if (object instanceof WhiteBox) {
                    string += "W  ";
                    continue;
                }
                if (object instanceof BlueBox) {
                    string += "B  ";
                    continue;
                }
                if (object instanceof RedTopBox) {
                    if (this.getObjectAt(x, y - 1) == object) {
                        string += "R  ";
                    } else {
                        string += "r  ";
                    }
                ...
                if (object instanceof RedBottomBox) {
                    if (this.getObjectAt(x, y - 1) == object) {
                        string += "r  ";
                    } else {
                        string += "R  ";
...
renderer = new Render();
new RRSWJS(canvas, renderer, false);
